...
export default new Plugin("openai", "OpenAI", "Talk to ChatGPT on WhatsApp!")
  .configSchema(
    optional(
      object({
  ...
  .registerCommand({
    ...
    async handler({ data }) {
      return await this.api.askAi(data);
    },
  })
  .registerCommand({
    name: "summarise",
    ...
    async handler({ message, data }) {
      ...
      if (message.hasQuotedMsg) {
        ...
        const completion =
          await this.api.whatsappMessageToChatCompletionMessage(
            quotedMsg,
            null,
            false,
          );
      ...
      const response = this.api.returnResponse(
        completion.choices[0].message.content,
      );
  ...
  .registerCommand({
    ...
    async handler({ message }) {
      if (!message.hasQuotedMsg) {
        throw new CommandError("reply to a message you want to summarise from");
      }
      ...
      for (let i = messages.length - 1; i >= 0; i--) {
        ...
        const completionMessage =
          await this.api.whatsappMessageToChatCompletionMessage(currentMessage);
...
`,
      ...
      const response = this.api
        .returnResponse(completion.choices[0].message.content)
        .replace(/^( *[*-] +)\*(\*.+?\*)\*/gm, "$1$2");
  ...
  .registerCommand({
    ...
    async handler({ message }) {
      ...
      await quotedMsg.reply(await this.api.transcribeMessage(quotedMsg));
    },
  })
  .registerCommand({
    name: "generate",
    ...
    async handler({ message, data, sender }) {
      ...
      const cache = this.api.getCached(hash, true);
      ...
      if (cache) {
        ...
        this.api.setCache(hash, Buffer.from(imageData, "base64"), true);
      }

      await message.reply(new MessageMedia("image/png", imageData), undefined, {
        caption,
      });
    },
  })
  .registerCommand({
    ...
    async handler({ message, data }) {
      ...
      this.api.setCache(hash, resultData, true);
